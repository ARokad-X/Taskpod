    ...
    <>
      <div className={`${TI_CLASSES.wrapper} ${borderColor}`}>        
        <div className={TI_CLASSES.leftContainer}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline gap-2 mb-1 flex-wrap">
              <h3 className={`${TI_CLASSES.titleBase} ${isCompleted ? 'text-brand-muted line-through' : 'text-brand-text'}`}>
                {task.title}
              </h3>
                ...
                <div className="space-y-1 sm:space-y-2 pt-1">
                  {subtasks.map((st, i) => (
                    <div key={i} className="flex items-center gap-2 group/subtask">
                      ...
                      />
                      <span className={`text-sm truncate ${st.completed ? 'text-brand-muted line-through' : 'text-brand-muted group-hover/subtask:text-brand-purple'} transition-colors duration-200`}>{st.title}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className={TI_CLASSES.rightContainer}>
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map(opt => (
                  <button
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-white/5 flex items-center gap-2 transition-colors duration-200"
                  >
                    {opt.icon}{opt.label}
                  </button>
                ))}
          ...
          <div>
            <div className={`${TI_CLASSES.dateRow} ${task.dueDate && isToday(new Date(task.dueDate)) ? 'text-brand-coral' : 'text-brand-muted'}`}>              
