    ...
    <div className="min-h-screen bg-brand-surface-light">
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-brand-coral to-brand-purple flex items-center justify-center text-white text-2xl font-bold shadow-md">
            {profile.name ? profile.name[0].toUpperCase() : "U"}
          </div>
          <div>
            <h1 className="text-3xl font-bold text-brand-text">Account Settings</h1>
            <p className="text-brand-muted text-sm">Manage your profile and security settings</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <section className={SECTION_WRAPPER}>
            <div className="flex items-center gap-2 mb-6">
              <UserCircle className="text-brand-purple w-5 h-5" />
          ...
          <section className={SECTION_WRAPPER}>
            <div className="flex items-center gap-2 mb-6">
              <Shield className="text-brand-purple w-5 h-5" />
              <h2 className="text-xl font-semibold text-brand-text">Security</h2>
            </div>
            <form onSubmit={changePassword} className="space-y-4">
              {securityFields.map(({ name, placeholder }) => (
                <div key={name} className={INPUT_WRAPPER}>
                  <Lock className="text-brand-purple w-5 h-5 mr-2" />
                  <input
                    type="password"
                    placeholder={placeholder}
